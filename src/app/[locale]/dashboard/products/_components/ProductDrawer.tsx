...
import { CalendarDays, Clock, ImageIcon, Package, Tag, X } from 'lucide-react';
...
export function ProductDetailsDrawer({
  ...
  const primaryMedia =
    displayProduct.media?.find((m) => m.is_primary) ||
    displayProduct.media?.[0];

  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-full overflow-y-auto border-white/10 bg-zinc-950 p-0 text-white sm:max-w-xl">
        <div className="relative h-64 w-full border-b border-white/10 bg-zinc-900">
          {primaryMedia ? (
            <img
